...
if __name__ == '__main__':
    ...
    # print(f'case ids: {case_ids}') ## Fahim: uncommented to check the case ids
    for c in case_ids:
        shutil.copy(join(brats_data_dir, c, c + "-t1n.nii.gz"), join(imagestr, c + '-0000.nii.gz'))
        shutil.copy(join(brats_data_dir, c, c + "-t1c.nii.gz"), join(imagestr, c + '-0001.nii.gz'))
        shutil.copy(join(brats_data_dir, c, c + "-t2w.nii.gz"), join(imagestr, c + '-0002.nii.gz'))
        shutil.copy(join(brats_data_dir, c, c + "-t2f.nii.gz"), join(imagestr, c + '-0003.nii.gz'))
    ...
    generate_dataset_json(out_base,
                          ...
                          labels={
                              'background': 0,
                              'resection cavity': (1,),
                              'enhancing tumour tissue': (2,),
                              'non-enhancing tumour core': (3,),
                              'surrounding non-enhancing FH': (4,)
                          },
