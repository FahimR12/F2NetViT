...
class util:
    def __init__(self, data_dir):
        ...
        self.LGG18 = MyConfig.LGG18
        self.HGG18 = MyConfig.HGG18
        self.brats19 = MyConfig.brats19_test_data
    ...
    def move_directories(source, destination):
        ...
        if 'HGG' in source:   
            files = os.listdir(source)[:MyConfig.Testset.HGG_files]
            for dir in files:
                src_path = os.path.join(source, dir)
                dest_path = os.path.join(destination, dir)
                shutil.move(src_path, dest_path)
        else:
            files = os.listdir(source)[:MyConfig.Testset.LGG_files]
            for dir in files:
                src_path = os.path.join(source, dir)
                dest_path = os.path.join(destination, dir)
                shutil.move(src_path, dest_path)
